...
> 注：命中率衡量"给定与资料相关的问题时，检索器对正确文档的召回能力"，是检索环节的代理指标；实际问答质量还取决于大模型生成环节。把 `docs/` 替换成你自己的资料后重跑 `python eval.py` 即可复现。
...
**如何生成这两张截图（本地，约 1 分钟）：**
1. 启动应用：先设好 Key（`$env:ZHIPU_API_KEY="你的key"`）再 `python app.py`；
2. 浏览器打开 http://127.0.0.1:5000：
   - 截一张**整体界面**（上传区 + 提问框 + 资料列表）→ 保存为 `images/web-ui.png`；
   - 上传一份资料并提问，截一张**问答结果**（回答 + 来源引用）→ 保存为 `images/demo-qa.png`；
3. 把两张图放进 `images/` 目录，告诉我一声，我帮你启用上面的引用并提交。
