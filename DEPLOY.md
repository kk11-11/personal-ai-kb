...
## 2. ModelScope 魔搭创空间（国内首选，Docker，免费档）

国内 CDN 快、免费（2 核 CPU + 16G 内存档），支持自定义 Docker，适合"已有东西想展示出去"。

> ⚠️ **前置必做（否则创建界面里根本看不到 Docker 入口）**：在魔搭 **绑定阿里云账号 + 完成实名认证**。Docker 类型强制要求此项，没做的话"运行环境 / Docker 类型"选项会被直接隐藏 —— 你"找不到运行环境"大概率就是这个原因。

> **关键认知**：魔搭的 Docker 类型**不是**在创建表单里点"Docker 单选"选出来的，而是由项目根目录 `ms_deploy.json` 里的 `sdk_type: "docker"` 声明的。正确做法是「上传含 ms_deploy.json 的文件夹」或「Git 推送含该文件的代码」，平台自动识别为 Docker 类型，不会再让你手选。

1. 确认已绑定阿里云 + 实名（魔搭设置页可查）。
2. 创空间列表 → **创建创空间** → 选 **"编程式创空间"**（或"快速部署并创建"模式）→ 上传**整个项目文件夹**（必须含根目录 `ms_deploy.json` + `Dockerfile`）。平台读取 `ms_deploy.json` 的 `sdk_type=docker` 自动归类为 Docker 类型。
   - 备选：先创建空空间，再 `git clone` 后用 git 推送代码（同样要带 `ms_deploy.json`）。
3. **端口必须为 7860**：本项目默认即监听 `7860`（无需再设 `PORT`，保持平台路由端口一致即可）。
4. 项目根目录已放好 `ms_deploy.json`（已随代码提交），内容：
   ```json
   {
     "sdk_type": "docker",
     "port": 7860,
